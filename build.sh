...
if [ $# -eq 0 ]; then
  ...
  echo '       build.sh'
  echo 'SYNOPSIS'
  echo '       ./build.sh <moodle_version>.<build_version>'
  echo 'DESCRIPTION'
  ...
  echo "                  ./build.sh 3.11.4.001"
  exit
fi

RELEASE_VERSION=$1
docker image rm -f $FULL_IMAGE_NAME:$RELEASE_VERSION
docker build . --progress plain -t $FULL_IMAGE_NAME:$RELEASE_VERSION --build-arg RELEASE_VERSION=$RELEASE_VERSION
