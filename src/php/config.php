...
require_once 'config_sentry.php';
...
$CFG->dbtype = get_env('CFG_DBTYPE', 'pgsql');
...
$CFG->dboptions = [
    // should persistent database connections be used? set to 'false' for the most stable
    // setting, 'true' can improve performance sometimes
    'dbpersist' => get_env_bool('CFG_DBPERSIST', 'false'),

    // should connection via UNIX socket be used? if you set it to 'true' or custom path
    //  here set dbhost to 'localhost', (please note mysql is always using socket
    //  if dbhost is 'localhost' - if you need local port connection use '127.0.0.1')    
    'dbsocket' => get_env_bool('CFG_DBSOCKET', 'false'),
    ...
    'dbhandlesoptions' => get_env_bool('CFG_DBHANDLESOPTIONS', 'false'),

    // MySQL has partial and full UTF-8 support. If you wish to use partial UTF-8
    // (three bytes) then set this option to 'utf8_unicode_ci', otherwise this option
    // can be removed for MySQL (by default it will use 'utf8mb4_unicode_ci'. This option should
    // be removed for all other databases.
    'dbcollation' => get_env('CFG_DBCOLLATION', 'utf8mb4_unicode_ci'),

    // On PostgreSQL, this option sets a limit on the number of rows that are fetched into
    // memory when doing a large recordset query (e.g. search indexing). Default is 100000.
    // Uncomment and set to a value to change it, or zero to turn off the limit. You need to
    // set to zero if you are using pg_bouncer in 'transaction' mode (it is fine in 'session'
    // mode).
    'fetchbuffersize' => get_env_int('CFG_DBFETCHBUFFERSIZE', '100000'),

    // Set connect timeout in seconds. Not all drivers support it.
    'connecttimeout' => get_env('CFG_DBCONNECTTIMEOUT', null),
...
$CFG->admin = get_env('CFG_ADMIN', 'admin');
...
if ($CFG->session_handler_class == '\core\session\memcached') {
    ...
    $CFG->session_memcached_lock_expire = get_env_int('CFG_SESSION_MEMCACHED_LOCK_EXPIRE', 7200); // Ignored if PECL memcached is below version 2.2.0
    $CFG->session_memcached_lock_retry_sleep = get_env_int('CFG_SESSION_MEMCACHED_LOCK_RETRY_SLEEP', 150); // Spin-lock retry sleeptime (msec). Only effective for tuning php-memcached 3.0.x (PHP 7)
}
...
if ($CFG->session_handler_class == '\core\session\redis') {
    ...
    $CFG->session_redis_auth = get_env('CFG_SESSION_REDIS_AUTH', '');
    $CFG->session_redis_prefix = get_env('CFG_SESSION_REDIS_PREFIX', '');
    $CFG->session_redis_acquire_lock_timeout = get_env_int('CFG_SESSION_REDIS_ACQUIRE_LOCK_TIMEOUT', 120);
    $CFG->session_redis_lock_expire = get_env_int('CFG_SESSION_REDIS_LOCK_EXPIRE', 7200);
    $CFG->session_redis_lock_retry = get_env_int('CFG_SESSION_REDIS_LOCK_RETRY', 100);

    // Use the igbinary serializer instead of the php default one. Note that phpredis must be compiled with
    //  igbinary support to make the setting to work. Also, if you change the serializer you have to flush the database!
    $CFG->session_redis_serializer_use_igbinary = get_env_bool('CFG_SESSION_REDIS_SERIALIZER_USE_IGBINARY', false);

    // Optional, possible values are:
    // 'gzip' - PHP GZip compression
    // 'zstd' - PHP Zstandard compression
    $CFG->session_redis_compressor = get_env('CFG_SESSION_REDIS_COMPRESSOR', 'none');
}
...
$CFG->session_update_timemodified_frequency = get_env_int('CFG_SESSION_UPDATE_TIMEMODIFIED_FREQUENCY', 20);
...
// like AOL.
$CFG->tracksessionip = get_env_bool('CFG_TRACKSESSIONIP', true);
...
$CFG->reverseproxy = get_env_bool('CFG_REVERSEPROXY', 'false');

// Enable when using external SSL appliance for performance reasons.
// Please note that site may be accessible via http: or https:, but not both!
$CFG->sslproxy     = get_env_bool('CFG_SSLPROXY', 'false');
...
$development = get_env_bool('CFG_DEVELOPMENT', 'false');
